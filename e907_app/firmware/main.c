...
// FIXME: 注释send_data_per_second函数后，uvc传输过程中e907跑飞
void send_data_per_second(int sockfd, struct sockaddr_in *client_addr, socklen_t *client_addr_len)
{
    char ReadBuff[50];
    while (1) {
        sendto(sockfd, ReadBuff, sizeof(ReadBuff), 0, (struct sockaddr *)client_addr, *client_addr_len);
    }
}
